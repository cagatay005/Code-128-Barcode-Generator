...
int save_bmp(const char *filepath, const Code128Data *barcode, int height, int scale) {
    ...
    // 24-bit RGB (3 bayt/piksel) kullaniyoruz. Padding (bosluk) hesabini yapiyoruz.
    int row_size = (width * 3 + 3) & (~3); 
    int data_size = row_size * height;
    
    // Dosya Basligini (14 Bayt) dolduruyoruz
    BMPFileHeader fileHeader = {0};
    fileHeader.bfType = 0x4D42; // Hex olarak "BM" (Bitmap) karakterlerine karsilik gelir
    fileHeader.bfSize = sizeof(BMPFileHeader) + sizeof(BMPInfoHeader) + data_size;
    fileHeader.bfOffBits = sizeof(BMPFileHeader) + sizeof(BMPInfoHeader);
    
    // Bilgi Basligini (40 Bayt) dolduruyoruz
    ...
    // Basliklari diske yaz (struct padding kapattigimiz icin tam 54 bayt yazilacak)
    fwrite(&fileHeader, sizeof(BMPFileHeader), 1, file);
    fwrite(&infoHeader, sizeof(BMPInfoHeader), 1, file);
    
    // Performans icin: Barkodun her satiri aynidir. 
    // Tum resmi tek tek hesaplamak yerine 1 satir olusturup bunu 'height' kere yazdiracagiz.
    unsigned char *row_buffer = (unsigned char *)calloc(row_size, 1);
    
    int byte_index = 0;
    for (size_t i = 0; i < barcode->length; i++) {
        // pattern '1' ise siyah (RGB: 0,0,0), '0' ise beyaz (RGB: 255,255,255) cizecegiz.
